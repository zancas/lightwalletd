...
FROM golang:1.14.3 AS lightwalletd_base
...
ARG     ZCASH_CONF_DIR=/srv/$LWD_USER/.zcash
ARG     ZCASH_CONF=$ZCASH_CONF_DIR/zcash.conf
RUN     mkdir $ZCASH_CONF_DIR
SHELL   ["/bin/bash", "-c"]
...
rpcuser=xxxxx\n\
rpcpassword=xxxxx\n\
rpcbind=172.16.238.10' > $ZCASH_CONF
